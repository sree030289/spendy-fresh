...
//   // OPTIMIZATION: Add static cache for expensive operations
//   private static expenseCache = new Map<string, { data: any; timestamp: number }>();
//   private static balanceCache = new Map<string, { data: any; timestamp: number }>>();
  
//   // Clear cache every 10 minutes to prevent memory leaks
//   private static cacheCleanerInitialized = false;
  
//   private static initCacheCleaner() {
//     if (!this.cacheCleanerInitialized) {
//       setInterval(() => {
//         const now = Date.now();
//         const CACHE_DURATION = 600000; // 10 minutes
        
//         for (const [key, value] of this.expenseCache.entries()) {
...
//   // OPTIMIZATION: Cache invalidation when data changes
//   private static invalidateUserCache(userId: string) {
//     for (const key of this.expenseCache.keys()) {
...
//   private static invalidateGroupCache(groupId: string) {
//     for (const key of this.expenseCache.keys()) {
...
//     // OPTIMIZATION: Initialize cache cleaner and invalidate cache after adding expense
//     this.initCacheCleaner();
//     this.invalidateGroupCache(expenseData.groupId);
    
//     // Invalidate user caches for all involved users
//     this.invalidateUserCache(expenseData.paidBy);
//     expenseData.splitData.forEach(split => {
//       this.invalidateUserCache(split.userId);
//     });
...
//   static async getGroupExpenses(groupId: string, limitCount: number = 50): Promise<Expense[]> {
//   try {
//     // OPTIMIZATION: Add limit and caching
//     const cacheKey = `group_expenses_${groupId}`;
//     const cached = this.expenseCache.get(cacheKey);
    
//     if (cached && Date.now() - cached.timestamp < 300000) { // 5 min cache
//       console.log('📦 Using cached group expenses for:', groupId);
//       return cached.data;
//     }
...
//       orderBy('createdAt', 'desc'),
//       limit(limitCount) // Add limit to reduce reads
//     );
...
//     // Cache the result
//     this.expenseCache.set(cacheKey, { data: expenses, timestamp: Date.now() });
...
//   // REAL-TIME LISTENERS - OPTIMIZED VERSIONS
//   static onGroupExpenses(groupId: string, callback: (expenses: Expense[]) => void): () => void {
//     // OPTIMIZATION: Use polling service instead of expensive real-time listener
//     const { PollingService } = require('@/services/optimizations/PollingService');
    
//     const fetchExpenses = async () => {
//       return await this.getGroupExpenses(groupId, 25); // Reduced limit
//     };
...
//   // OPTIMIZATION: Keep original real-time method for critical updates only
//   static onGroupExpensesRealTime(groupId: string, callback: (expenses: Expense[]) => void): () => void {
//     const expensesQuery = query(
//       collection(db, 'expenses'),
//       where('groupId', '==', groupId),
//       orderBy('createdAt', 'desc'),
...
//     // OPTIMIZATION: Use polling for notifications instead of real-time
//     const { PollingService } = require('@/services/optimizations/PollingService');
...
//     // Poll every 2 minutes for notifications
//     return PollingService.startPolling(
...
//     // OPTIMIZATION: Use polling for friend requests
//     const { PollingService } = require('@/services/optimizations/PollingService');
...
//     // Poll every 3 minutes for friend requests (less frequent)
//     return PollingService.startPolling(
//       `friend_requests_${userId}`,
//       fetchRequests,
//       callback,
//       180000, // 3 minutes
//       90000   // 90 second cache
//     );
//   }

//   static onFriends(userId: string, callback: (friends: Friend[]) => void): () => void {
//     // OPTIMIZATION: Add limit to reduce reads
//     const friendsQuery = query(
//       collection(db, 'friends'),
//       where('userId', '==', userId),
//       orderBy('lastActivity', 'desc'),
//       limit(100) // Limit to 100 friends max
//     );
...
//   try {
//     // OPTIMIZATION: Add caching for expensive user expense queries
//     const cacheKey = `user_expenses_${userId}_${limitCount}`;
//     const cached = this.expenseCache.get(cacheKey);
    
//     if (cached && Date.now() - cached.timestamp < 180000) { // 3 min cache for user expenses
//       console.log('📦 Using cached user expenses for:', userId);
//       return cached.data;
//     }
    
//     // First get user's groups (with reduced limit if possible)
...
//     // OPTIMIZATION: Reduce max groups queried and add stricter limits
//     const maxGroups = Math.min(groupIds.length, 8); // Reduced from 10 to 8
//     const adjustedLimit = Math.min(limitCount, 50); // Cap at 50 max
    
//     // Get expenses from user's groups
//     const expensesQuery = query(
//       collection(db, 'expenses'),
//       where('groupId', 'in', groupIds.slice(0, maxGroups)),
//       orderBy('createdAt', 'desc'),
//       limit(adjustedLimit)
//     );
...
//     const userExpenses = await this.getUserExpenses(userId, 100); // OPTIMIZED: Reduced from 1000 to 100
...
//           orderBy('createdAt', 'desc'),
//           limit(100) // OPTIMIZED: Reduced from 1000 to 100 per batch
//         );
...
//     const allExpenses = await this.getUserExpenses(userId, filters.limit || 50); // OPTIMIZED: Reduced default from 1000 to 50
...
// }> {
//   try {
//     const expenseList = expenses || await this.getUserExpenses(userId, 100); // OPTIMIZED: Reduced from 1000 to 100
...
//     const expenses = await this.getUserExpenses(userId, 100); // OPTIMIZED: Reduced from 1000 to 100
...
// }> {
//   try {
//     const allExpenses = await this.getUserExpenses(userId, 100); // OPTIMIZED: Reduced from 1000 to 100
