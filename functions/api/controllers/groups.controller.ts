...
export class GroupsController {
  ...
  static async createGroup(req: AuthenticatedRequest, res: Response): Promise<void> {
    try {
      console.log('🔍 Creating group with request body:', JSON.stringify(req.body, null, 2));
      console.log('🔍 User info:', { id: req.user!.id, fullName: req.user!.fullName, email: req.user!.email });
      ...
      const groupData: Omit<Group, 'id'> = {
        ...
        members: [{
          ...
          role: 'admin',
          balance: 0,
          joinedAt: new Date(), // Use regular Date object, not server timestamp
          isActive: true
        }],
        totalExpenses: 0,
      ...
      // Add initial members if provided (filter out null/undefined values)
